...
const removeComponentsFromContentItem = (
  ...
  targetComponentPaths.forEach((targetPath: string) => {
    ...
    contentRegionMutators.removeComponent(
      clonedContentItem,
      targetPath,
      results.pathTrackers[clonedContentItem._path]
    )

    results.reportSuccess(clonedContentItem, targetPath, "DELETE");
  })
}
...
export function createCleanupEditor(
  ...
  const editor: EditorFunc = (contentItem) => {
    ...
    try {
      ...
      results.markError(contentItem, lastAttempted.componentPath, "DELETE", e);
      return contentItem;
    }
  };
